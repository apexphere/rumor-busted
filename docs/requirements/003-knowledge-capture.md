...
When a saying is verified (CONFIRMED or PARTIALLY TRUE), save it to the user's local knowledge base. Knowledge is stored per saying, not per article.

## What Gets Saved

Each verified saying becomes a knowledge entry:

| Field | Description |
|-------|-------------|
| `saying` | What the article said |
...
**Example entry:**
```
Saying: "Google's A2A protocol complements MCP, not replaces it"
Verdict: confirmed
...
- ❌ BUSTED sayings — user doesn't need to remember false info
- ❌ UNVERIFIED sayings — not enough evidence
- ❌ OPINION sayings — subjective, not knowledge
- ❌ The original article itself — just the verified sayings
...
When sayings are verified and saved:
> "Saved 2 verified sayings to your knowledge base."
...
Response: List of recent verified sayings with summaries.
...
- [ ] Verified sayings (confirmed/partially_true) are saved automatically
- [ ] Each saying is stored with verdict, summary, sources, and context
- [ ] User can view their saved knowledge
- [ ] Busted/unverified/opinion sayings are NOT saved
